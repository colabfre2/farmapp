...
@section('title', 'Tambah Barang Masuk Borongan')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold text-dark font-quicksand mb-0">📥 Catat Barang Masuk Borongan</h3>
    <a href="{{ route('admin.stock.in.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill fw-bold shadow-sm px-4">← Kembali</a>
</div>
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <p class="text-muted small mb-0"><i class="bi bi-info-circle me-1"></i> Form ini untuk mencatat penambahan stok produk secara borongan.</p>
...
<form id="bulkMovementForm" method="POST" action="{{ route('admin.stock.in.store-bulk') }}">
    @csrf
        ...
        <div class="row g-4">
            <div class="col-lg-7">
                ...
                <div class="row g-3 mb-2">
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Alasan Masuk</label>
                        <select name="movements[__INDEX__][reason]" class="form-select rounded-3 py-2">
                            <option value="">-- Pilih Alasan --</option>
                            <option value="Restock">📦 Restock</option>
                            <option value="Return dari buyer">↩️ Return dari buyer</option>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function showStockInfo(row) {
        ...
        const currentStock = selectedOpt.dataset.stock;
        infoDiv.style.display = 'block';
        infoDiv.textContent = `Stok saat ini: ${currentStock} unit. Stok akan bertambah setelah disimpan.`;
    }

    function attachRowEvents(row) {
        const select = row.querySelector('.product-select');
        select.addEventListener('change', () => showStockInfo(row));
