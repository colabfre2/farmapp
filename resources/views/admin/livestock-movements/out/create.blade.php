...
@section('title', 'Tambah Ternak Keluar Borongan')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold text-dark font-quicksand mb-0">📤 Catat Ternak Keluar / Penjualan Borongan</h3>
    <a href="{{ route('admin.livestock-movements.out.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill fw-bold shadow-sm px-4">← Kembali</a>
</div>
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <p class="text-muted small mb-0"><i class="bi bi-info-circle me-1"></i> Form ini untuk mencatat ternak keluar (penjualan offline/mutasi) secara borongan.</p>
...
<form id="bulkMovementForm" method="POST" action="{{ route('admin.livestock-movements.out.store-bulk') }}">
    @csrf
        ...
        <div class="row g-4">
            <div class="col-lg-7">
                    ...
                    <select name="movements[__INDEX__][livestock_id]" class="form-select livestock-select rounded-3 py-2" required>
                        ...
                        @foreach($livestocks as $item)
                            <option value="{{ $item->id }}" data-stock="{{ $item->quantity }}">
                                {{ $item->name }} (Stok tersedia: {{ $item->quantity }} ekor)
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="row g-3 mb-2">
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Alasan Keluar <span class="text-danger">*</span></label>
                        <select name="movements[__INDEX__][reason]" class="form-select rounded-3 py-2" required>
                            ...
                            <option value="Konsumsi Pribadi">🍽️ Konsumsi Pribadi</option>
                            <option value="Lainnya">📌 Lainnya</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Catatan / Pembeli</label>
                        <input type="text" name="movements[__INDEX__][notes]" class="form-control rounded-3 py-2" placeholder="cth: Dibeli Pak Budi...">
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-5">
                <div class="card border-0 h-100 shadow-none" style="border-radius: 12px; background: #f8fafc; border: 1px dashed #cbd5e1 !important;">
                    <div class="card-body p-3 p-md-4">
                        ...
                        <div class="mb-2">
                            <label class="form-label fw-semibold">Jumlah Keluar (ekor) <span class="text-danger">*</span></label>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function checkStock(row) {
        ...
        const maxStock = parseInt(selectedOpt.dataset.stock || 0);
        ...
        if (inputQty > maxStock) {
            ...
            warning.textContent = `⚠️ Melebihi stok! Maksimal tersedia ${maxStock} ekor.`;
        } else {
